...
def main():
    ...
    target_common = f"""				ASSETCATALOG_COMPILER_APPICON_NAME = AppIcon;
				...
				INFOPLIST_KEY_NSLocationWhenInUseUsageDescription = "Your location is used to offer you jobs near your truck rather than near your yard, and is shared with a customer only while you are working their job. Without Always, the map your customer is watching stops updating whenever you switch to another app.";
				INFOPLIST_KEY_NSLocationAlwaysAndWhenInUseUsageDescription = "Two things, both while you are signed in. Between jobs we keep one approximate position, overwritten each time, so you are offered work near your truck rather than near your yard. While you are actually on a job, your position is shared with that customer so they can watch you arrive, and is kept with the job as a record. Turn the switch off in Alerts at any time.";
